...
export type AdStage = "new" | "qualifying" | "booked" | "disqualified";

/** What the ad promised. Doubles as the bar every lead is later scored against. */
...
export interface AdCreative {
  /** The hook's underlying idea, so angles can be tested rather than just wording. */
  angle: string;
  headline: string;
  primaryText: string;
  description: string;
...
/** Where a DM program runs. Ads campaigns use AdPlatform instead. */
export type DmChannel = "instagram" | "linkedin" | "x" | "facebook";

/** A cold opener template. Placeholders: {{first_name}}, {{detail}}. */
...
export interface AdCampaign {
  id: string;
  /** 'ads' = inbound from paid traffic, 'dm' = outbound cold DMs. */
  kind: "ads" | "dm";
  channel: DmChannel | null;
  openers: DmOpener[];
  follow_ups: DmFollowUp[];
  name: string;
  platform: AdPlatform;
...
export interface AdLead {
  ...
  email: string;
  phone: string;
  /** Social identity for DM prospects — the dedupe key prefers it over the name. */
  handle: string;
  channel: DmChannel | null;
  source: string;
  stage: AdStage;
